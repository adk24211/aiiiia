...
/// Generate an id stamped with a specific time, for tests.
pub fn new_at(kind: Kind, millis: u64) -> String {
    let mut raw = [0u8; 16];
...
pub fn is(kind: Kind, id: &str) -> bool {
    let Some(body) = id.strip_prefix(kind.prefix()).and_then(|r| r.strip_prefix('_')) else {
        return false;
    };
...
pub fn time_of(id: &str) -> Option<u64> {
...
mod tests {
    ...
    fn the_timestamp_survives_a_round_trip() {
        for stamp in [0, 1, 1_700_000_000_000, (1u64 << 48) - 1] {
